...
class LogStash::Outputs::Http < LogStash::Outputs::Base
  ...
  config :format, :validate => ["json", "json_batch", "form", "message", "logdna_batch"], :default => "json"
  ...
  def register
    ...
    if @content_type.nil?
      case @format
        ...
        when "logdna_batch" ; @content_type = "application/json"
        when "message" ; @content_type = "text/plain"
      end
    end

    @is_batch = %w(json_batch, logdna_batch).include?(@format)
  ...
  def event_body(event)
    ...
    elsif @format == "logdna_batch"
      LogStash::Json.dump({ lines: event.map {|e| map_event(e) } })
    else
